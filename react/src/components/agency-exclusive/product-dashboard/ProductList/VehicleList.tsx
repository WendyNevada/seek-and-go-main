...
const VehicleList = () => {
    ...
    useEffect(() => {
        const fetchVehicle = async () => {
            try {
                const response = await axiosClient.post('/v1/GetActiveVehicleByAgencyIdWithoutQty', {
                        ...
                        rows[rows.length - 1].push(
                            <div key={vehicle.ref_vehicle_id} className='flex-1'>
                                <Card className='w-64 shadow-lg mt-10 mr-16'>
                                    ...
                                    <CardHeader>
                                        <CardTitle className='w-[200px] truncate min-h-8'>{vehicle.vehicle_name}</CardTitle>
